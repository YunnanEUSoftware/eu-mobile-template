<template>
	<view class="main-list oBorder" :style="{ 'height': height, 'border-radius': borderRadius }">
		<!-- 文本框 -->
		<input 
			class="main-input" 
			:value="value" 
			...
			@input="$emit('input', $event.target.value,id)"
		...
		<!-- 倒计时 -->
		<!-- <slot></slot> -->
		<view v-if="_isShowCode&&!_isShowPass" :class="['vercode',{'vercode-run': second>0}]" @click="setCode" id="elementId">{{ getVerCodeSecond }}</view>
