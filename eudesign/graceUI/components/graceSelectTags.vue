<template>
	<view class="grace-select-tags">
		<view class="">
			...
			:style="{
				width : itemWidth, 'background' : item.checked ? selectedColor+' !important' : bgColor, fontSize:fontSize, 
				height:height, lineHeight:height, borderRadius:borderRadius,  color : item.checked ? fontActiveColor : fontColor
			}" 
			@tap.stop="graceSelectChange(index)" v-for="(item, index) in tagsData" :key="index">{{item.name}}</view>
		</view>
	</view>
</template>
<script>
export default{
	props: {
		...
		selectedColor : { type: String, default: "#1890ff"},
...
<style scoped>
.grace-select-tags{   overflow:auto; }
.grace-select-tags > view { display:flex;  flex-wrap:wrap; }
